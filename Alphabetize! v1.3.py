...
class AlphabetizeApp:
    def __init__(self, root):
        self.root = root
        self.root.title("Alphabetize! v1.3")
        ...
        help_text.insert(tk.END, "Welcome to Alphabetize! v1.3\n\n", "center bold")
        help_text.insert(tk.END, "TO USE:\n", "center")
        help_text.insert(tk.END, "Choose a target folder with the 'Select Folder' button and press 'Alphabetize!'\n\nThe Alphabetize! function will move the target folder's contents into Alphabetized! sub-folders 'A'-'Z' with a '#' folder for contents starting with numbers and an '.Other' folder for contents starting with any other character.\n\n", "center")
        help_text.insert(tk.END, "NOTES:\n", "center")
        help_text.insert(tk.END, "1. Folders within the target folder will be moved without altering their contents.\n\n2. The 'Undo' function will only delete empty sub-folders named '#', 'A'-'Z', or '.Other' upon completion.")
    ...
    def alphabetize_files(self):
        ...
        for filename in os.listdir(dest_folder):
            if filename != os.path.basename(__file__):
                ...
                elif first_char.isalpha() and 'A' <= first_char <= 'Z':
                    subfolder_path = os.path.join(dest_folder, first_char.upper())
                else:
                    subfolder_path = os.path.join(dest_folder, ".Other")
    ...
    def undo_files(self):
        ...
        for root, dirs, files in os.walk(dest_folder, topdown=False):
            for dir_name in dirs[:]:
                if dir_name == "#" or (len(dir_name) == 1 and dir_name.isalpha()) or dir_name == ".Other":
